...
export default function SignIn({ onSwitchToSignUp, onSignInSuccess, onForgotPassword }: SignInProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
      ...
      <div className="w-full max-w-[750px] h-auto max-h-[85vh] bg-[#1a1f26] rounded-2xl overflow-hidden flex flex-col md:flex-row shadow-2xl relative">

        {/* Close Button - Mapped to onSwitchToSignUp (or should act as close?) in this context strictly for consistency, though usually Sign In needs a way to exit. Reusing switch for now or external close if provided, but prop only has switch. */}
        {/* Note: Original SignIn didn't have a close prop, just switch. I'll add a 'close' via switch or just visual consistency. */}
        <button
        ...
        <div className="w-full md:w-[60%] bg-[#13181D] p-4 md:p-5 overflow-y-auto custom-scrollbar flex flex-col justify-center">
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-gray-600 text-[9px] font-bold uppercase">or sign in with email</span>
            <div className="h-[1px] bg-gray-800 flex-1"></div>
          </div>
          ...
          <form className="space-y-3" onSubmit={handleSubmit}>
            <div className="space-y-0.5">
              ...
              <input
                ...
                placeholder="Email"
                className="w-full bg-[#1A1F26] border border-gray-700/50 rounded-lg px-2.5 py-1.5 text-white text-[11px] placeholder-gray-600 focus:border-[#84CC16] focus:outline-none transition-colors"
              />
            </div>
            ...
            <div className="space-y-0.5">
              <div className="flex justify-between items-center">
                ...
                <button type="button" onClick={onForgotPassword} className="text-[#FF4B0F] text-[10px] hover:underline">Forgot?</button>
              </div>
              <input
                type="password"
                name="password"
